...
module.exports = (req, res) => {
    res.setHeader('Access-Control-Allow-Origin', '*'); // CORS追加
    ...
    if (req.url === '/api/cesium-token') {
        console.log('Cesium token check:', cesiumAccessToken); // デバッグログ追加
        if (cesiumAccessToken) {
            res.status(200).send(cesiumAccessToken);
        } else {
            res.status(500).json({ error: 'Cesium access token not set or invalid' });
        }
        return;
    }
    // TLEデータを取得
